...
(function () {
  ...
  async function pollApi() {
    if (typeof window.wcFetchYard !== 'function') {
      updateSourcePill();
      return;
    }
  ...
  function renderStats() {
    const s = el('stat-settled'); if (s) s.textContent = eth(settledTotal) + ' ETH';
    const n = el('stat-settlements'); if (n) n.textContent = Number(settlementCount).toLocaleString('en-US');
    const h = el('stat-hounds'); if (h) h.textContent = String(hounds.length);
  }
